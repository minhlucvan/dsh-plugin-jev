...
function buildSkillContent(service: JevService): string {
  const lines: string[] = [
    ...
    '- the answer is a position on a spectrum you can describe (`jev_score`);',
    '- the answer is yes or no and the probability itself is the signal (`jev_check`);',
    '- you need several judgements about the same evidence (`jev_ask`);',
    '- you are unsure how to approach a task and want its shape named (`jev_reason`);',
    '- you must choose between several candidates and keep their scores apart (`jev_compare`).',
    '',
